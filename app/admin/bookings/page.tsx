...
export default async function Bookings(){
  ...
  return <section className="adminShell"><div className="eyebrow">RESERVATIONS</div><h1>Bookings</h1><NewBooking cameras={cameras||[]} userId={user.id}/><div className="adminPanel"><div className="panelHeading"><div><h2>Booking history</h2><p>Accepted external rental requirements appear in the Sub-Rentals checklist.</p></div><Link className="button ghost" href="/admin/sub-rentals">Open Sub-Rentals</Link></div>{(books||[]).map((b:any)=>{const ext=b.booking_subrentals||[];const pending=ext.filter((x:any)=>!["confirmed","received","returned","cancelled"].includes(x.status)).length;return <Link href={`/admin/bookings/${b.id}`} className="bookingRow tall clickableBooking" key={b.id}><div><b>{b.booking_code} · {b.production_name||b.customers?.company_name||b.customers?.name}</b><span>{b.project_name} · {new Date(b.start_at).toLocaleString("en-IN")} → {new Date(b.end_at).toLocaleString("en-IN")}</span><span>{(b.booking_cameras||[]).map((x:any)=>x.cameras?.camera_code).filter(Boolean).join(", ")}</span>{ext.length>0&&<span className={pending?"externalBookingWarning":"externalBookingOk"}>External equipment: {ext.length} · {pending?`${pending} need sourcing`:"all confirmed/handled"}</span>}</div><div className="bookingRowRight"><em className={`status ${b.status}`}>{b.status.replaceAll("_"," ")}</em><span>Open →</span></div></Link>})}</div></section>;
}
