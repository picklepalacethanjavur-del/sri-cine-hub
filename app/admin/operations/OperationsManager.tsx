...
export function OperationsManager({bookings,userId}:{bookings:any[];userId:string}){
  ...
  const expected=(b?.booking_cameras||[]).map((x:any)=>x.cameras?.qr_code).filter(Boolean);
  ...
  async function checkout(e:React.FormEvent<HTMLFormElement>){
    e.preventDefault();
    if(!b||busy)return;
    const missing=expected.filter((x:string)=>!scanned.includes(x));
    if(missing.length)return setMsg(`Missing scans: ${missing.join(", ")}`);
  ...
  return <>
    <div className="adminPanel formPanel">
      ...
      <select value={bookingId} disabled={!!busy} onChange={e=>{setBookingId(e.target.value);setScanned([]);setMsg("")}}>
    ...
    {b&&["reserved","confirmed","preparing"].includes(b.status)&&<form className="adminPanel formPanel" onSubmit={checkout}>
      <h2>QR checkout</h2>
      <AssetScanner onCode={code=>setScanned(s=>s.includes(code)?s:[...s,code])}/>
      <div className="scanSummary"><b>Expected {expected.length}</b><span>Scanned {scanned.length}</span></div>
      ...
      <button className="button gold" disabled={!!busy}>{busy==="checkout"?"Checking Out…":"Complete checkout"}</button>
    </form>}
    ...
    {msg&&<div className={msg.includes("completed")||msg.includes("generated")?"successBox":"errorBox"}>{msg}</div>}
