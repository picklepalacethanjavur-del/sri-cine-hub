import {notFound} from "next/navigation";import Link from "next/link";import {AdminNav} from "@/components/AdminNav";import {RequestDocuments} from "@/components/RequestDocuments";import {requireStaff} from "@/lib/auth";import {QuotationEditor} from "./QuotationEditor";import {DeleteQuotationButton} from "./DeleteQuotationButton";
export default async function QuotationDetail({params}:{params:Promise<{id:string}>}){
 ...
 return <section className="adminShell v6AdminShell"><div className="eyebrow">QUOTATION</div><div className="detailTitleRow"><div><h1>{q.quotation_code}</h1><p>{q.customers?.company_name||q.customers?.name||request?.company_name||request?.name||"Customer not entered"}</p></div><div className="detailActions"><Link className="button ghost" href={`/admin/quotations/${id}/print`}>View Document</Link>{isAdmin&&<DeleteQuotationButton quotationId={id} quotationCode={q.quotation_code} quoteRequestId={q.quote_request_id||null}/>}</div></div><AdminNav/>{q.quote_request_id&&<RequestDocuments requestId={q.quote_request_id} attachments={attachments||[]} userId={user.id} compact/>}<QuotationEditor quotation={q} items={items||[]} request={request} cameras={cameras||[]} accessories={accessories||[]} kits={kits||[]} rates={rates||[]} supplierItems={supplierItems||[]} isAdmin={isAdmin}/></section>;
}
